...
descrange2 <- paste(des[63:67], collapse = " ")
...
descrange3 <- paste(des[58:62], collapse = " ")
...
result_df <- data.frame(Description = c(descrange, descrange2, descrange3))
...
# Assuming product1 is your HTML document or webpage content
# Replace "your_selector_here" with the appropriate CSS selector for your case
rate1 <- product1 %>%
  html_nodes("span.a-size-base#acrCustomerReviewText") %>%
  html_text()

# Extract only numeric data using regular expression
numeric_data <- as.numeric(gsub("\\D", "", rate1))

# Format numeric data with commas
data1 <- format(numeric_data, big.mark = ",")

# Print the formatted numeric data
print(data1)
...
# Extract only numeric data using regular expression
numeric_data2 <- as.numeric(gsub("\\D", "", rate2))

# Format numeric data with commas
data2 <- format(numeric_data2, big.mark = ",")
...
# Extract only numeric data using regular expression
numeric_data<- as.numeric(gsub("\\D", "", rate3))

# Format numeric data with commas
data3 <- format(numeric_data, big.mark = ",")
...
# Extract ratings for each product
ratings <- c(data1, data2, data3)

# Create a data frame with ratings
...
# Extract the text from the specified span
review_text <- product1 %>%
  html_node("span.a-size-base.review-text") %>%
  html_text()

# Remove newline characters
review_text <- str_replace_all(review_text, "\n", "")

# Print the extracted review text without newline characters
cat(review_text, sep = " ")
...
# Print the extracted review text without newline characters
cat(review_text3, sep = " ")

DATA FRAME FOR REVIEWS

{r}
reviews_df <- data.frame(
  ReviewText = c(review_text, review_text2, review_text3)
)
